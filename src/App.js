        ...
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-red-50 p-6 text-center">
                ...
                <div className="bg-white p-4 rounded border border-red-200 text-left w-full max-w-md overflow-auto mb-4">
                 ...
                 {appError.toLowerCase().includes("permission") && (
                    <div className="bg-yellow-100 p-3 rounded text-sm text-yellow-800 mb-4">
                        <strong>Check Rules:</strong> This is likely a Firestore Permission error.
                    </div>
                )}
...
const Header = ({ title, logoUrl }) => (
    <div className="flex items-center justify-between mb-4 px-4 pt-4">
        ...
        {/* v3.0 Marker to verify update */}
        <h1 className="text-2xl font-bold text-gray-900 flex-grow truncate">{title} <span className="text-xs text-gray-400">(v3.0)</span></h1>
    </div>
);
...
const DashboardScreen = ({ restaurant, userId }) => {
    ...
    // Helper to handle potentially mixed date formats
    ...
    const calculatePredictions = useCallback(async () => {
        ...
        try {
            ...
            // FIX: Show raw error message
            setPredictionError(`DEBUG: ${error.message}`);
        } finally {
